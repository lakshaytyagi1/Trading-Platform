...
import com.trade.CryptoTrading.Repository.PaymentOrderRepository;
import com.trade.CryptoTrading.domain.PaymentMethod;
import com.trade.CryptoTrading.domain.PaymentOrderStatus;
...
public class PaymentServiceImpl implements PaymentService{
    ...
    public PaymentResponse createStripePaymentLink(User user, Long amount, Long orderId) {
        ...
        Stripe.apiKey = stripeSecretKey; // Make sure stripeSecretKey is set somewhere

        try {
            // Stripe expects the amount in paise (for INR)
            Long amountInPaise = amount * 100;

            SessionCreateParams params = SessionCreateParams.builder()
                    .addPaymentMethodType(SessionCreateParams.PaymentMethodType.CARD)
                    .setMode(SessionCreateParams.Mode.PAYMENT)
                    .setSuccessUrl("http://localhost:8080/wallet?status=success&orderId=" + orderId)
                    .setCancelUrl("http://localhost:8080/wallet?status=cancel")
                    .addLineItem(
            ...
            System.out.println("Error creating Stripe payment link: " + e.getMessage());
            throw new RuntimeException(e);
        }
    }
}
